...
\begin{tikzpicture}[remember picture, overlay]
  ...
  \node[anchor=center, text width=13cm, align=center] at ($(current page.center)+(0,-1.3)$) {
    {\color{white!85!black}\fontsize{11}{14}\selectfont
     ...
     para clasificar el dataset Iris con tres algoritmos distintos}
  };
...
\begin{enumerate}[leftmargin=1.6em, itemsep=3pt, label=\textcolor{cTealDeep}{\textbf{\arabic*.}}]
  ...
  \item Tres ramas paralelas, una por cada \textbf{algoritmo de clasificación} (detallados en la siguiente sección), cada una con su propio \textbf{ClassifierPerformanceEvaluator} y \textbf{TextViewer} para revisar sus métricas por separado.
\end{enumerate}

\figuraMarcada{img/pipeline-completo.png}{0.98}{Pipeline completo armado en KnowledgeFlow: un origen de datos, validación cruzada, y tres algoritmos de clasificación evaluados en paralelo.}

\subsection{\faCogs\ \ Algoritmos de clasificación evaluados}
Ejecuté y comparé tres algoritmos con enfoques muy distintos:
...
\figuraMarcada{img/resultado-multilayer-perceptron.png}{0.95}{Resultado de Multilayer Perceptron: 97.33\% de precisión (146/150 instancias correctas), el mejor de los tres.}

\subsection{\faChartBar\ \ Comparación de resultados}
Reuniendo las métricas de los tres algoritmos:

\figuraMarcada{img/grafico-comparacion-precision.png}{0.95}{Precisión global de los tres algoritmos evaluados con validación cruzada de 10 pliegues.}

La diferencia entre los tres es pequeña (menos de 2 puntos porcentuales), pero la matriz de confusión de cada uno revela \emph{dónde} se equivoca cada algoritmo:
...
Esto no es casualidad: el propio dataset documenta que \textit{setosa} es linealmente separable de las otras dos especies, mientras que \textit{versicolor} y \textit{virginica} no lo son entre sí. Los tres algoritmos ``descubrieron'' ese mismo patrón de forma independiente.

\subsection{\faTree\ \ Visualización del árbol J48}
De los tres algoritmos, solo J48 se puede dibujar como un árbol simple (Random Forest son 100 árboles combinados, y la red neuronal son pesos numéricos, no reglas). Usé el componente \textbf{GraphViewer} conectado a la salida \texttt{graph} de J48 para visualizarlo:
...
\begin{evidenciabox}
Ejecuté el pipeline completo de principio a fin sin errores: los 12 componentes del flujo (carga, asignación de clase, validación cruzada, 3 clasificadores, 3 evaluadores, 3 visores de texto y el GraphViewer) terminaron correctamente.
\end{evidenciabox}
...
\begin{itemize}[leftmargin=1.6em, itemsep=6pt]
  ...
  \item[\textcolor{cMint}{\faCheckCircle}] \textbf{Multilayer Perceptron} obtuvo la mejor precisión (97.33\%), seguido de \textbf{J48} (96\%) y \textbf{Random Forest} (95.33\%) -- aunque la red neuronal fue notablemente más lenta de entrenar.
  \item[\textcolor{cMint}{\faCheckCircle}] Las estadísticas descriptivas (correlación de atributos), el árbol de J48 y las matrices de confusión de los tres algoritmos contaron exactamente la misma historia de forma independiente: el tamaño del pétalo es lo que realmente distingue a las especies, y \textit{Iris-setosa} es trivialmente separable mientras que \textit{versicolor} y \textit{virginica} se solapan.
  \item[\textcolor{cMint}{\faCheckCircle}] Entendí una limitación real de las herramientas de visualización de modelos: no todo algoritmo se puede dibujar como árbol, y reconocer por qué (ensamble vs. red neuronal vs. árbol único) es tan importante como saber usar la herramienta.
\end{itemize}
